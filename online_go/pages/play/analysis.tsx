...
import { Settings, Flag, User, Cpu, ChevronLeft, ChevronRight, Activity, MessageSquare, ListTree, Undo2, Lightbulb, LineChart, ArrowLeft, Layers, X, Zap, Target, Clock, GraduationCap, RotateCcw } from 'lucide-react';
...
export default function AnalysisBoard() {
    ...
    const [timeTier, setTimeTier] = useState<'fast' | 'medium' | 'long' | 'unlimited'>('unlimited');
    const [timePresetId, setTimePresetId] = useState<string>('19_u_1');
    ...
    const TIME_CONFIG = {
        9: {
            ...
            unlimited: { label: '∞ Infinite', presets: [{ id: '9_u_1', label: 'No time limit' }] }
        },
        13: {
            ...
            unlimited: { label: '∞ Infinite', presets: [{ id: '13_u_1', label: 'No time limit' }] }
        },
        19: {
            ...
            unlimited: { label: '∞ Infinite', presets: [{ id: '19_u_1', label: 'No time limit' }] }
        }
    };
    ...
    // --- UI OPTIONS STATE --- //
    const [activeTab, setActiveTab] = useState('analysis'); // 'analysis' | 'tree'
    ...
    // Rehydrate persisted configuration
    useEffect(() => {
        try {
            const savedMaxHints = localStorage.getItem('katagoSetup_maxHints');
            const savedStrat = localStorage.getItem('katagoSetup_hintStrategy');
            const savedHints = localStorage.getItem('katagoSetup_showHints');
            const savedTerritory = localStorage.getItem('katagoSetup_showTerritory');
            const savedGraph = localStorage.getItem('katagoSetup_graphMode');

            if (savedMaxHints) setMaxHints(parseInt(savedMaxHints));
            if (savedStrat) setHintStrategy(savedStrat);
            if (savedHints) setShowHints(savedHints === 'true');
            if (savedTerritory) setShowTerritory(savedTerritory === 'true');
            if (savedGraph) setGraphMode(savedGraph);
        } catch (e) {
            // Safe fallback
        }
    }, []);

    // Sync state mutations cleanly
    useEffect(() => {
        ...
        localStorage.setItem('katagoSetup_maxHints', maxHints.toString());
        localStorage.setItem('katagoSetup_graphMode', graphMode);
    }, [hintStrategy, showHints, showTerritory, maxHints, graphMode]);
            ...
            setTime(prev => {
                ...
                if (main === 0) {
                    if (timeControl === 'byoyomi' && periods > 0) {
                        ...
                        // In Analysis Mode, we NEVER timeout. Simply freeze the clock at 0.
                        return prev;
                    }
                }
    ...
    const handleStartMatch = async () => {
        ...
        // Immediately analyze the empty sandbox so hints exist before the first move
        requestTutorAnalysis(createEmptyBoard(boardSize), 'black', 0);
    };
    ...
    const startHeroMatch = async () => {
        ...
        if (timeTier !== 'unlimited') {
            const preset = (TIME_CONFIG as any)[loadedSize as 9 | 13 | 19]?.[timeTier]?.presets.find((p: any) => p.id === timePresetId);
            if (preset) {
                let parts = preset.label.split('+');
                if (parts.length === 2) {
                    let mainStr = parts[0].trim();
        ...
        try {
            ...
            if (nextPlayer === 'white') {
                ...
                requestTutorAnalysis(loadedBoard, nextPlayer, blackCount + whiteCount);
            }
        } catch (err) {
    ...
    const requestTutorAnalysis = async (currentBoard, toPlayColor, currentPly) => {
        ...
        const visitsMap = { 1: 100, 2: 500, 3: 1500, 4: 5000, 5: 15000 };
        const dynamicVisits = user?.analysis_level ? visitsMap[user.analysis_level as keyof typeof visitsMap] : 1500;
        ...
        const payload = {
            ...
            max_visits: dynamicVisits, // Modulate engine evaluation organically based on global parameters
            max_top_moves: 5,
    ...
    const hints = React.useMemo(() => {
        ...
        if (hintStrategy === 'balanced') return sortedHints.slice(0, tuitionMode ? targetCandidates : maxHints);
        ...
        if (filtered.length === 0) return legalHints.slice(0, maxHints);
        if (filtered.length < maxHints) {
            const backfill = legalHints.filter(h => !filtered.some(f => f.move === h.move)).slice(0, maxHints - filtered.length);
            return [...filtered, ...backfill];
        }
        return filtered.slice(0, maxHints);
    }, [rawHints, hintStrategy, boardSize, board, displayBoard, gameState, viewingMove, currentPlayer, maxHints]);
    ...
    const handleIntersectionClick = async (row: number, col: number) => {
        if (gameState === 'mistake_training') {
            return; // Irrelevant for standalone analysis mode
        }

        if (gameState === 'finished' || isBotThinking) {
            return;
        }
        ...
        const result = applyGoRules(board, currentPlayer, row, col, boardHistoryListRef.current);
        if (!result) {
            return;
        }
        ...
        if (currentPlayer === 'black') {
            ...
            setCaptures(prev => ({ ...prev, white: prev.white + result.capturedStones }));
        }

        setLastMove({ r: row, c: col });
        boardHistoryListRef.current.push(JSON.stringify(newBoard));

        const updatedHistory = [...moveHistory, { m: moveHistory.length + 1, c: toCoordinates(row, col, boardSize), p: currentPlayer === 'black' ? 'b' : 'w' }];
        ...
        // Fetch engine telemetry asynchronously for the new board state
        // This simulates the right-hand Analysis panel UI without automating the opponent.
        requestTutorAnalysis(newBoard, nextPlayer, updatedHistory.length);
    };
        ...
        <div className="min-h-[100dvh] bg-[#FDFCF9] text-gray-900 font-sans flex flex-col items-center justify-center pt-[80px] pb-[40px] px-4 relative overflow-x-hidden">
            ...
            <div className="w-full max-w-[1920px] flex flex-col items-center justify-center relative z-10 flex-1">
                ...
                <main className="w-full flex flex-col xl:flex-row gap-[24px] justify-center items-center">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center shrink-0 min-w-[300px] w-full xl:max-w-[70vw] order-1 xl:order-2 gap-[24px] z-20 overflow-visible xl:px-4">
                        <div className="w-full flex items-center justify-center relative my-2 sm:my-4">
                            ...
                            <div className={`absolute top-[52%] z-[5] hidden xl:flex flex-col pointer-events-none transition-all duration-[800ms] ease-[cubic-bezier(0.4,0,0.2,1)] origin-right ${showAnalysis ? 'gap-[120px]' : 'gap-[20px]'}`}
                                style={{
                                    left: `calc(50% + ${boardPx / 2}px + ${showAnalysis ? '-65' : '60'}px)`,
                                    transform: `translateY(-50%) scale(${showAnalysis ? 0.55 : 1})`
                                }}>
                                {/* White Bowl */}
                    ...
                    <div className={`z-30 pt-[10px] hidden xl:flex flex-col shrink-0 transition-all duration-[800ms] ease-[cubic-bezier(0.4,0,0.2,1)] overflow-visible order-3 ${showAnalysis ? 'w-[420px] opacity-100' : 'w-[200px] opacity-0 pointer-events-none'}`}>
                        <div className={`w-full xl:w-[420px] bg-gradient-to-b from-white/80 to-[#F4F0EC]/92 backdrop-blur-[16px] border border-white/35 rounded-[28px] shadow-[0_24px_60px_rgba(25,18,12,0.12),_0_8px_24px_rgba(25,18,12,0.08),_inset_0_1px_0_rgba(255,255,255,0.7)] p-[24px] flex flex-col shrink-0 h-[760px] min-h-[760px] max-h-[760px] relative overflow-hidden z-20`}>
                            ...
                            {/* GLOBAL HEADER ROW */}
                            <div className="flex items-center justify-between mb-5 shrink-0 relative z-50 w-full">
                                ...
                                <button
                                    onClick={() => {
                                        sessionStorage.removeItem('heroBoardState');
                                        sessionStorage.removeItem('heroMatchState');
                                        setMoveHistory([]);
                                        setMoveCount(0);
                                        setCurrentPlayer('black');
                                        setBoard(createEmptyBoard(boardSize));
                                    ...
                                    className="flex items-center gap-1.5 px-2.5 py-1.5 bg-[#F4F0EC]/80 hover:bg-white text-[#6E675F] hover:text-[#3A342E] rounded-[8px] text-[10px] font-bold tracking-widest uppercase transition-colors shadow-sm border border-[#503C28]/10"
                            ...
                            {gameState === 'mistake_training' ? (
                                        ...
                                        <div className="bg-white/55 border border-[#503C28]/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.6),_0_8px_20px_rgba(30,20,10,0.05)] rounded-[20px] p-5 flex flex-col items-center justify-center text-center gap-4 relative overflow-hidden">
                                            ...
                                            <div className="flex w-full gap-3 relative z-10">
                                                <button onClick={() => {
                                                    ...
                                                    setWinrate(null);
                                                    setGameState('setup');
                                                }} className={`py-3 flex justify-center items-center gap-2 bg-gradient-to-b from-[#1a1a1a] to-[#0f0f0f] border border-white/5 shadow-[0_12px_30px_rgba(0,0,0,0.35),_inset_0_1px_0_rgba(255,255,255,0.08)] text-white/95 font-bold rounded-[16px] hover:from-[#2a2a2a] hover:to-[#1a1a1a] transition-colors ${reviewMistakes.length > 0 ? 'w-full' : 'flex-1'}`}>
                                ...
                                <React.Fragment>
                                    ...
                                    <div className="flex gap-2 border-b border-[#F0F0F0]/0 shrink-0 mb-6">
                                        {/* Chat tab removed for analysis mode */}
                                        ...
                                        {activeTab === 'analysis' && (
                                            <div className="flex flex-col gap-4 px-2 mt-4 relative z-10">
                                                ...
                                                <div className="bg-white/55 rounded-[20px] p-4 text-center border border-[#503C28]/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.6),_0_8px_20px_rgba(30,20,10,0.05)] flex flex-col gap-3 relative overflow-hidden">
                                                    ...
                                                    <div className="flex flex-col gap-1 relative z-10 w-full mt-2 bg-white/50 p-2.5 rounded-[12px] border border-[#503C28]/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.6)]">
                                                        <div className="flex justify-between items-center text-[11px] font-bold text-[#6E675F] px-1 mb-1">
                                                            <span className="uppercase tracking-widest">Top Engine Moves</span>
                                                            <span className="text-[#3A342E] bg-[#EBE5DF] px-2 py-0.5 rounded-[4px]">{maxHints}</span>
                                                        </div>
                                                        <input
                                                            type="range"
                                                            min="3"
                                                            ...
                                                            className="w-full h-1.5 bg-[#EBE5DF]/80 rounded-lg appearance-none cursor-pointer accent-[#C38A4A]"
                                                        />
                                                    </div>
                ...
                `
                                ...
                                <div className="setup-panel flex flex-col relative w-full h-auto max-h-[90vh] overflow-hidden">
                                    <div className="hero-zone shrink-0">
                                        ...
                                        <div className="hero-title">Analysis Board</div>
                                        <div className="hero-subtitle">Free play & explore variations</div>
                                    </div>
                                    <div className="hero-content flex flex-col overflow-y-auto pr-1 flex-1 z-10 space-y-3" style={{ scrollbarWidth: 'none' }}>
                                        ...
                                        <div className="setup-section">
                                            ...
                                            <div className="flex gap-2 w-full mt-1">
                                                {(['fast', 'medium', 'long', 'unlimited'] as const).map(tier => (
                                                    <div
                                                        key={tier}
                                                        onClick={() => {
                                                            setTimeTier(tier);
                                    ...
                                    <div className="pt-2 z-20 pb-0 shrink-0">
                                        <button onClick={handleStartMatch} className="cta-btn w-full h-[48px] rounded-[16px] font-bold text-[14px] tracking-wide transition-all flex items-center justify-center gap-2 outline-none">
                                            <svg className="w-4 h-4 opacity-90" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> Launch Analysis Board
                                        </button>
                                    </div>
                                </div> {/* END setup-panel */}
